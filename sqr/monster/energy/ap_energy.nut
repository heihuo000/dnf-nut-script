...
{
	appendage.sq_AddFunctionName("proc", "proc_appendage_NewStyleEnergy")
	appendage.sq_AddFunctionName("onStart", "onStart_appendage_NewStyleEnergy")
	appendage.sq_AddFunctionName("onEnd", "onEnd_appendage_NewStyleEnergy")
}
...
function onEnd_appendage_NewStyleEnergy(appendage)
{
...
//            parentObj.flushSetStatePacket();
    ...
    if (apd)
    {
        apd.getVar("monsterEnergyDrawUI").set_vector(0,-1);
        apd.getVar("monsterEnergyDrawUI").set_vector(1,-1);
        apd.getVar("monsterEnergyDrawUI").set_vector(2,-1);
        apd.getVar("monsterEnergyDrawUI").set_vector(3,-1);
    }

}

function proc_appendage_NewStyleEnergy(appendage)
{
	if(!appendage) {
		return;		
	}
    ...
    if (pChr)
    {
        pChr = sq_GetCNRDObjectToSQRCharacter(pChr);
        local apd = getComboUiAppendage(pChr);
        apd.getVar("monsterEnergyDrawUI").set_vector(0,monsterIconID);
        apd.getVar("monsterEnergyDrawUI").set_vector(1,atkID);
        apd.getVar("monsterEnergyDrawUI").set_vector(2,time);
        apd.getVar("monsterEnergyDrawUI").set_vector(3,currentT);
    }
...
function onStart_appendage_NewStyleEnergy(appendage)
{
	if(!appendage) {
		return;
	}
