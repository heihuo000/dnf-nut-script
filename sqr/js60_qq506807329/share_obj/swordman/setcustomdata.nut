function setCustomData_po_qq506807329new_swordman_24370(obj, receiveData) {
	...
	case 250:
		...
		setCurrentAnimationFromCutomIndex(obj, 73);
		...
		break;
	case 126:
		sq_SetCurrentAttackInfoFromCustomIndex(obj, 57);
		setCurrentAnimationFromCutomIndex(obj, 72);
		local bonus = receiveData.readDword();
		local size = receiveData.readDword() / 100;
		local count = receiveData.readDword();
		local areaSizeRate = size.tofloat();

		obj.getVar("subType").clear_vector();
		obj.getVar("subType").push_vector(count);
		...
		sq_SetCurrentAttackInfo(obj, attackInfo);
		sq_SetCurrentAttackBonusRate(attackInfo, bonus.tointeger());
		obj.setTimeEvent(0, (900 / count).tointeger(), 0, false);
		break;
	case 229:
	...
	case 234:
		...
		case 5:
			...
			local lightDamage = receiveData.readDword();
			local duration = receiveData.readDword();
			sq_SetChangeStatusIntoAttackInfoWithEtc(otvlUaC33uZaqJ7I0O3Ume4, 0, ACTIVESTATUS_LIGHTNING, prob.tointeger(), level, duration, lightDamage,0);
			sq_SetChangeStatusIntoAttackInfoWithEtc(O_LRW5cDVVQdNH15XSPK, 0, ACTIVESTATUS_LIGHTNING, prob.tointeger(), level, duration, lightDamage,0);
			sq_SetChangeStatusIntoAttackInfoWithEtc(zB9PvpsNIpM9JGUl, 0, ACTIVESTATUS_LIGHTNING, prob.tointeger(), level, duration, lightDamage,0);
			break;
		case 1:
	...
	case 47:
		...
		local aYVzdFsTrz0D9kiq_eWuN = receiveData.readDword();
		...
		local OUIPGgxWtgcQn1a = sq_GetCustomAttackInfo(obj, TZGRuH4ubPESkRW9cTXl9n);
		sq_SetCurrentAttackBonusRate(OUIPGgxWtgcQn1a, receiveData.readDword());
		sq_SetCurrentAttackInfo(obj, OUIPGgxWtgcQn1a);
		obj.getVar().clear_vector();
		obj.getVar().push_vector(QKpIC0sJHDd52YAbvmNC);
