...
function setState_po_qq506807329new_swordman_24370(obj, PdS2a7ApkG, eD_buMT_5jAj)
{
 if(!obj) return;
 obj.getVar("state").clear_vector(); 
 obj.getVar("state").push_vector(PdS2a7ApkG);
 ...
 if(jSpLKuTeGSVlByBPKnxC && isSameObject(obj, jSpLKuTeGSVlByBPKnxC.getSource())) 
 ...
 obj.setCurrentPos(sq_GetVectorData(eD_buMT_5jAj, 0),
 ...
 sq_SetAttackInfoForceHitStunTime(sq_GetCurrentAttackInfo(obj),
 obj.getVar().get_vector(4)
 ); 
 ...
 RemoveAllFlash(obj); 
 if(obj.isMyControlObject())
 sq_flashScreen(obj, 0, 0, PG5dkLwkVDEot.getDelaySum(false), 130, sq_RGB(0, 0, 0), GRAPHICEFFECT_NONE, ENUM_DRAWLAYER_BOTTOM); 
 break;
 }
 }
 ...
 obj.setCurrentPos(FnUFL8OeyfKg.getXPos(), FnUFL8OeyfKg.getYPos() + 1, FnUFL8OeyfKg.getZPos() + sq_GetHeightObject(FnUFL8OeyfKg) / 2); 
 else
 obj.setCurrentPos(_YOLMPJEs387UynZ.get_vector(2), _YOLMPJEs387UynZ.get_vector(3) + 1, _YOLMPJEs387UynZ.get_vector(4)); 
 ...
 {
 local LPJLg1UQ9QgqLBAvzzWSHL = sq_GetCNRDObjectToActiveObject(sq_GetObject(obj, sq_GetVectorData(eD_buMT_5jAj, RRJsvJZLOOrp), sq_GetVectorData(eD_buMT_5jAj, RRJsvJZLOOrp + 1))); 
 ...
 if(sq_IsGrabable(obj, LPJLg1UQ9QgqLBAvzzWSHL) && !sq_IsFixture(LPJLg1UQ9QgqLBAvzzWSHL)) 
 sq_MoveToAppendage(LPJLg1UQ9QgqLBAvzzWSHL, obj, LPJLg1UQ9QgqLBAvzzWSHL, 0, 0, LPJLg1UQ9QgqLBAvzzWSHL.getZPos() / -1 + 50, 500, true, lx_kPyj_ymeQJ_F); 
 obj.getVar().push_obj_vector(LPJLg1UQ9QgqLBAvzzWSHL); 
 }
 }
 sq_SetCurrentPos(obj, sq_GetVectorData(eD_buMT_5jAj, RRJsvJZLOOrp), sq_GetVectorData(eD_buMT_5jAj, RRJsvJZLOOrp + 1), 0); 
