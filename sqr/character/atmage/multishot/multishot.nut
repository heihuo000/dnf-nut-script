TIMER_MULTI_SHOT_CREATE_MAGIC_BALL	<- 0; // ???
TIMER_MULTI_SHOT_INPUT_COUNT		<- 1; // ??? ???? ??? ???.
TIMER_MULTI_SHOT_WATING_STAND		<- 2; // ??????? ????? ?????? ?????

VAR_MULTI_SHOT_CURRENT_COUNT <- 0;

ENUM_MULTI_SHOT_SUBSTATE_CHARGE   <- 0; // ???? ?? ???
ENUM_MULTI_SHOT_SUBSTATE_HORIZON  <- 1; // ???? ?? ????
ENUM_MULTI_SHOT_SUBSTATE_VERTICAL <- 2; // ???? ?? ????
ENUM_MULTI_SHOT_SUBSTATE_DIAGONAL <- 3; // ???? ?? ????
ENUM_MULTI_SHOT_SUBSTATE_FLOOR	  <- 4; // ?????? ???
...
function checkExecutableSkill_MultiShot(obj)
{
	...
	if(b_useskill) {		
		...
		if(obj.getState() == STATE_JUMP || obj.getState() == STATE_JUMP_ATTACK)
...
function checkCommandEnable_MultiShot(obj)
{
	...
	if(state == STATE_ATTACK) {
		return obj.sq_IsCommandEnable(SKILL_MULTI_SHOT); 
	}
...
function onSetState_MultiShot(obj, state, datas, isResetTimer)
{
	...
	if(subState == ENUM_MULTI_SHOT_SUBSTATE_FLOOR || subState == ENUM_MULTI_SHOT_SUBSTATE_CHARGE) // ?????? ?????
...
{		
	...
	if(timeEventIndex == TIMER_MULTI_SHOT_CREATE_MAGIC_BALL) {		
		...
		if(count < max) {
			...
			local inputMax		= sq_GetIntData(obj, SKILL_MULTI_SHOT, 0); // 0:??? ??? ?? ???? 
			local gapByInputMin = sq_GetIntData(obj, SKILL_MULTI_SHOT, 1); // 1:??? ???? ms (??? ????) 
			local gapByInputMax = sq_GetIntData(obj, SKILL_MULTI_SHOT, 2); // 2:??? ???? ms (??? ????)
			local gap = 300;
			...
			if(currentElementalType == ENUM_ELEMENT_NONE)
			{
				...
				if(subState == ENUM_MULTI_SHOT_SUBSTATE_DIAGONAL || subState == ENUM_MULTI_SHOT_SUBSTATE_VERTICAL) // ????,????
...
function onEndCurrentAni_MultiShot(obj)
{
	...
	if(subState == ENUM_MAGIC_CANNON_SUBSTATE_CHARGE) {
		...
		local subState = ENUM_MAGIC_CANNON_SUBSTATE_DIAGONAL; // ?????? ??
...
function onKeyFrameFlag_MultiShot(obj, flagIndex)
{
	...
	if(flagIndex == 1) {	
		...
		obj.setTimeEvent(TIMER_MULTI_SHOT_INPUT_COUNT, 1000, 1,false); // ??? ???? ??? ???.
