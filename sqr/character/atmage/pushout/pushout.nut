ENUM_AMP_SUBSTATE_GRAB <-0 ;	// ???
ENUM_AMP_SUBSTATE_PUSH <-1 ;	// ?б?
ENUM_AMP_SUBSTATE_RELEASE <-2;	// ????

VAR_AMP_IS_GRAB				 <-0 //???? ???? ????? ü?
VAR_AMP_HOLD_REALSE			 <-1 //?????? ????
VAR_AMP_HOLD_REALSE_COMPLETE <-2 //???? ???
...
function checkExecutableSkill_PushOut(obj)
{
	...
	if(b_useskill) {
		obj.sq_IntVectClear();
		obj.sq_IntVectPush(ENUM_AMP_SUBSTATE_GRAB); // substate????		
...
function checkCommandEnable_PushOut(obj)
{
	...
	if(state == STATE_ATTACK) {
		// ?????忡???? ???????? ????? ????????. ?????:?????? [2012.04.20]
		return obj.sq_IsCommandEnable(SKILL_PUSH_OUT); // ?????忡???? ???????? ????? ????????. ?????:?????? [2012.04.20] obj.sq_IsCommandEnable(SKILL_PUSH_OUT);
	}
...
function onProc_PushOut(obj)
{
	...
	for(local i=0;i<objectsSize;++i)
	{
		...
		if(damager)
		{
			...
			if(obj.isCurrentCutomAniIndex(CUSTOM_ANI_PUSH_OUT_GRAB)) { // ??? ??? ??????? ??????.					
...
function onSetState_PushOut(obj, state, datas, isResetTimer)
{
	...
	if(subState == ENUM_AMP_SUBSTATE_GRAB) {
		...
		local pushingAttackPower = obj.sq_GetBonusRateWithPassive(SKILL_PUSH_OUT , STATE_PUSH_OUT, 0, 1.0);// ?о??? ?????
		local attackInfo		 = sq_GetCurrentAttackInfo(obj);		
		local pushingPower		 = sq_GetLevelData(obj, SKILL_PUSH_OUT, 1, level); // ?о??? ??.	
...
function onEndCurrentAni_PushOut(obj)
{
	...
	if(subState == ENUM_AMP_SUBSTATE_GRAB)	{
		...
		if(objectsSize <= 0) {
			...
			obj.sq_IntVectClear();
			obj.sq_IntVectPush(ENUM_AMP_SUBSTATE_PUSH); // substate????	
			obj.sq_AddSetStatePacket(STATE_PUSH_OUT , STATE_PRIORITY_USER, true);		
		}
	}
	else if(subState == ENUM_AMP_SUBSTATE_PUSH)
	{
		...
		obj.sq_IntVectClear();
		obj.sq_IntVectPush(ENUM_AMP_SUBSTATE_RELEASE); // substate????	
		obj.sq_AddSetStatePacket(STATE_PUSH_OUT , STATE_PRIORITY_USER, true);	
	}
	else if(subState == ENUM_AMP_SUBSTATE_RELEASE) 
	{
