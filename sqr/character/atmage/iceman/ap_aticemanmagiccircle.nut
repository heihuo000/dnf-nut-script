...
function proc_appendage_grab_icemagic(appendage)
{
	...
	if (obj.getState() != STATE_HOLD && obj.getState() != STATE_DIE && checkIsHoldAndDelayEffect(obj) )
	{
		//  ???????? ??????HOLD?? ????
...
function prepareDraw_appendage_grab_icemagic(appendage)
{
	...
	if(attacker.getState() == STATE_ICEMAN && attacker.getSkillSubState() == SUB_STATE_ICEMAN_3) // ???????
	{
