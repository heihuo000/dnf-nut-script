ENUM_MAGIC_CANNON_SUBSTATE_CHARGE   <- 0;  // ???
ENUM_MAGIC_CANNON_SUBSTATE_HORIZON  <- 1;  // ????(????)
ENUM_MAGIC_CANNON_SUBSTATE_VERTICAL <- 2; // ????(????)
ENUM_MAGIC_CANNON_SUBSTATE_DIAGONAL <- 3; // ????(????)
ENUM_MAGIC_CANNON_SUBSTATE_LAND		<- 4; // ????? ????? ?????.
...
function checkCommandEnable_MagicCannon(obj)
{
	...
	if(state == STATE_ATTACK || state == STATE_STAND || state == STATE_DASH || state == STATE_JUMP || state == STATE_JUMP_ATTACK)
	{
		return true;
	}
	return false;
