function setCustomData_po_bloodcastle_bloodwater(obj, receiveData)
{
	if(!obj)
		return;
...
//        local ani = obj.getCustomAnimation(15);
    ...
    }else if (id == 3)
    {
		...
		obj.getVar("max").push_vector(savT);

//        local ani = obj.getCustomAnimation(2);
		local ani = obj.getVar().GetAnimationMap("1", "passiveobject/zrr_skill/newmage/animation/thornvinehold/area/loopfloor_10.ani"); 
		...
		local size = 1.0;
		local sizeRate = receiveData.readDword() / 100.0;
...
//        local ani = obj.getCustomAnimation(9);
    ...
    }else if (id == 10)
    {
		...
		local bloodAtk = receiveData.readDword();
		local bloodleve = receiveData.readDword();
		local bloodtime = receiveData.readDword();
		local bloodRate = 100;
		...
		local ani = obj.getVar().GetAnimationMap("1", "passiveobject/zrr_skill/newmage/animation/painfulgarden/floor_08.ani"); 
		if(ani){
			obj.setCurrentAnimation(ani);
		}
		...
		attackInfo = sq_GetCurrentAttackInfo(obj);
		sq_SetCurrentAttackBonusRate(attackInfo, attackBonusRate);
		sq_SetChangeStatusIntoAttackInfo(attackInfo, 0, ACTIVESTATUS_BLEEDING, bloodRate, bloodleve, bloodtime,bloodAtk);
...
//
    ...
    {
		local attackBonusRateB = receiveData.readDword();
		local size = receiveData.readDword();
		...
		local ani = parentObj.sq_GetCustomAni(CUSTOM_ANI_SILHOUETTE_SHOOT_BODY);
		parentObj.sq_SetCurrentTimeByFrame(ani, 0);

		//local ani = obj.getVar().GetAnimationMap("1","character/mage/atanimation/silhouette_shoot_body.ani"); 
		
		obj.setCurrentAnimation(ani);
		ani.setSpeedRate( speed.tofloat() );
		local attackInfo = sq_GetCustomAttackInfo(obj, 8);
		if(isHorizen == 1){
			attackInfo = sq_GetCustomAttackInfo(obj, 9);
		}
		sq_SetCurrentAttackInfo(obj, attackInfo);
		...
		obj.sq_SetMoveParticle("Particle/silhouette.ptl", 0.0, 0.0);
		
		obj.getVar("atk").clear_vector();
		obj.getVar("atk").push_vector(attackBonusRateB);
		obj.getVar("atk").push_vector(size);
		obj.getVar("flag").clear_vector();
		obj.getVar("flag").push_vector(isHorizen);
...
function procAppend_po_bloodcastle_bloodwater(obj)
{
...
function onEndCurrentAni_po_bloodcastle_bloodwater(obj)
{
    local id = obj.getVar("id").get_vector(0);
    if (id == 1)
    {
...
function onKeyFrameFlag_po_bloodcastle_bloodwater(obj, flagIndex)
{
    local id = obj.getVar("id").get_vector(0);
    if (id == 10 && obj.getVar("flag").get_vector(0) == 0)
    {
        ...
        sq_SetCurrentAttackInfo(obj, attackInfo);
        attackInfo = sq_GetCurrentAttackInfo(obj);
        sq_SetCurrentAttackBonusRate(attackInfo, obj.getVar("value").get_vector(0));
...
function onAttack_po_bloodcastle_bloodwater(obj,damager, boundingBox, isStuck)
{
	if(!obj)	
		return;
	local id = obj.getVar("id").get_vector(0);
	...
	else if (id == 15)
	{
		if(!damager)	
			return;
		local sizeRate = damager.getObjectHeight();
		sizeRate = sizeRate.tofloat()/100.0;
		local objx = obj.getXPos();
		local objy = obj.getYPos() + 2;
		local objz = obj.getZPos();
		
		local ani = null;
		if (obj.getVar("flag").get_vector(0) == 1)
		{
			ani = sq_CreateAnimation("", "passiveobject/zrr_skill/atmage/animation/atsilhouette/silhouette_eventhorizongrab.ani");
		}
		else
		{
			ani = sq_CreateAnimation("", "passiveobject/zrr_skill/atmage/animation/atsilhouette/silhouette_boom.ani");
		}
    ...
    else if (id == 3)
    {
		if (obj.getVar("state").get_vector(0) == 0)
		{
			local append = "character/new_mage/cirrusarea/ap_cirrusarea.nut"
			if ( !CNSquirrelAppendage.sq_IsAppendAppendage(damager, append) )
			{
				local appendage = CNSquirrelAppendage.sq_AppendAppendage(damager, obj, SKILL_CIRRUSAREA  false, append, true);
				if(appendage)
				{
					...
					appendageinfo.setValidTime(obj.getVar("max").get_vector(2))
				}
			}
		}
		else
...
function onTimeEvent_po_bloodcastle_bloodwater(obj, timeEventIndex, timeEventCount)
{
    ...
    else if (id == 3)
    {
        if (timeEventIndex == 0)
        {
            if (timeEventCount > obj.getVar("max").get_vector(0))
            {
                ...
                sq_SetCurrentAttackInfo(obj, attackInfo);
                attackInfo = sq_GetCurrentAttackInfo(obj);
		   sq_SetCurrentAttackBonusRate(attackInfo, obj.getVar("max").get_vector(1));
                obj.resetHitObjectList();
