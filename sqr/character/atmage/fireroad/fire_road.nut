...
function onKeyFrameFlag_FireRoad(obj, flagIndex) {
	...
	if (skillSubState == 0) {
		...
		local pauseTime = obj.sq_GetIntData(SKILL_FIRE_ROAD, 0); // ??????? ????????(?e?)
		...
		local damage1 = obj.sq_GetBonusRateWithPassive(SKILL_FIRE_ROAD, STATE_FIRE_ROAD, 1, 1.0); // ?????1(%)
		local damage2 = obj.sq_GetBonusRateWithPassive(SKILL_FIRE_ROAD, STATE_FIRE_ROAD, 2, 1.0); // ?????2(%)
