...
HOLONG_LIGHT_STATE_BUFF     <- 2; // ??? ???? : ?????? ?????? ??????. : ?нú? ????????? 2???? ???????.
HOLONG_LIGHT_STATE_ATTACK   <- 3; // ???? : ?????? ????? ?????????? ?????? ???????.
HOLONG_LIGHT_STATE_DESTROY  <- 4; // ??????? ???????.
HOLONG_LIGHT_STATE_EXPLOSION <- 5; // ????


HOLONG_LIGHT_CUSTOM_ANI_MOVE	<- 0; // ????? ???
HOLONG_LIGHT_CUSTOM_ANI_DESTROY <- 1; // ????? ???
HOLONG_LIGHT_CUSTOM_ANI_SHOOT	<- 2; // ???? ??????? ???
HOLONG_LIGHT_CUSTOM_ANI_EXPLOSION <- 3; //???? ???
...
function onDestroyObject_po_ATHolongLight(obj, destroyObj)
{
	...
	if(!parentObj || isSameObject(parentObj,destroyObj)) { //?θ? ??ü?? ?????? ????? 	
		obj.sendStateOnlyPacket(HOLONG_LIGHT_STATE_DESTROY);
	}	
...
function setState_po_ATHolongLight(obj, state, datas)
{
	...
	if (state ==  HOLONG_LIGHT_STATE_BUFF) {	
		...
		timer.Reset();
		timer.Start(500,0);
		obj.setTimeEvent(0,lifeTime,1,false); // 0?? ?ε????? ????, 5????? ???? ???, 1????, ó???? ???????.
