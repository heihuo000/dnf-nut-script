function setCustomData_po_blooddance(obj, receiveData)
{
	if(!obj)
		return;
...
//        local ani = obj.getCustomAnimation(3);
        local ani = obj.getVar().GetAnimationMap("1", "passiveobject/zrr_skill/atmage/animation/aticecraft/icecraft_make.ani"); 
        obj.setCurrentAnimation(ani);
...
//17
	...
	local sizeRate = receiveData.readDword();
        local ani = obj.getVar().GetAnimationMap("1",  "passiveobject/zrr_skill/newmage/animation/dot/madthebig/attackend_00.ani"); 
        obj.setCurrentAnimation(ani);
    ...
    }else if (id == 11)
    {
        ...
        local range = receiveData.readDword()/2;
        local ani = obj.getVar().GetAnimationMap("1", 
	...
	obj.getVar("v").clear_vector();
        obj.getVar("v").push_vector( range );
    }else if (id == 12)
    {
        local atkTime = receiveData.readDword();
...
function procAppend_po_blooddance(obj)
{
    ...
    if (id == 23)
    {
    	...
    	local range = obj.getVar("v").get_vector(0);
    	setRangeObjectPosEx(obj, range, sq_GetDistancePos(0, obj.getDirection(), 0), 0, ENUM_DIRECTION_NEUTRAL, 0, 400, 200, 2);
    }
    else if (id == 22)
    {
    ...
    }else if (id == 15)
    {
        ...
        if (state == 0)
        {
            ...
            if (currentT < obj.getVar("values").get_vector(0))
            {
                ...
                if (tar)
                {
                    local vx = sq_GetUniformVelocity(startX, tar.getXPos(), currentT, obj.getVar("values").get_vector(0));
                    local vy = sq_GetUniformVelocity(startY, tar.getYPos(), currentT, obj.getVar("values").get_vector(0));
...
//                                object.setCurrentPos(0,0,1000);
					...
					catch(ex)
					{
                                ...
                                local masterAppendage = CNSquirrelAppendage.sq_AppendAppendage(object, mage, SKILL_HOTLOVE, false, "character/new_mage/hotlove/ap_hotlove_friend.nut", true);				 
                                
                                masterAppendage.sq_SetValidTime( obj.getVar("v").get_vector(5) );
                                if(mage){
                                   masterAppendage.setAppendCauseSkill(BUFF_CAUSE_SKILL, ENUM_CHARACTERJOB_MAGE, SKILL_HOTLOVE, sq_GetSkillLevel(mage, SKILL_HOTLOVE));
                                }else{
                                   masterAppendage.setAppendCauseSkill(BUFF_CAUSE_SKILL, ENUM_CHARACTERJOB_MAGE, SKILL_HOTLOVE, 1);
                                }
                                CNSquirrelAppendage.sq_AppendAppendageID(masterAppendage, object, mage, SKILL_HOTLOVE, true);
                                	...
                                	if (isFavoriteing(object) )
					{
						...
						casSped = casSped * 1.2;
                                ...
                                if(!change_appendage)
                                {
                                    change_appendage = masterAppendage.sq_AddChangeStatus("break",object, mage, 0, 0, false, 0);
...
function onEndCurrentAni_po_blooddance(obj)
{
    local id = obj.getVar("id").get_vector(0);
    if (id == 0)
    {
...
function onTimeEvent_po_blooddance(obj, timeEventIndex, timeEventCount)
{
    ...
    }else if (id == 8)
    {
        ...
        if (state == 1)
        {
            ...
            local objectManager = obj.getObjectManager();
                ...
                {
                    local object = objectManager.getCollisionObject(i);
                    CNSquirrelAppendage.sq_RemoveAppendage(object, "character/new_mage/hotlove/ap_hotlove_enemy.nut");
                }
            sq_SendDestroyPacketPassiveObject(obj);
        }
    }else if (id == 4)
...
function onKeyFrameFlag_po_blooddance(obj, flagIndex)
{
    local id = obj.getVar("id").get_vector(0);
...
function onAttack_po_blooddance(obj,damager, boundingBox, isStuck)
{
...
//            local ani = obj.getCustomAnimation(1);
    ...
    if (id == 0)
    {
        ...
        if(mage && damager && object.isObjectType(OBJECTTYPE_ACTIVE))
        {
