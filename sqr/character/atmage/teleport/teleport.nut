...
function checkExecutableSkill_Teleport(obj)
{
	...
	if(b_useskill)
	{
		...
		obj.sq_IntVectClear();
		obj.sq_IntVectPush(SUB_STATE_TELEPORT_0); // substate????
...
function onSetState_Teleport(obj, state, datas, isResetTimer)
{
	...
	if(substate == SUB_STATE_TELEPORT_0) {
		...
		// SUB_STATE_TELEPORT_3 ????????? ???
	}
	else if(substate == SUB_STATE_TELEPORT_4) {
		// SUB_STATE_TELEPORT_4 ????????? ???
	}
...
function prepareDraw_Teleport(obj)
{
	...
	if(substate == SUB_STATE_TELEPORT_0) {
		...
		// SUB_STATE_TELEPORT_1 ????????? ???
	}
	else if(substate == SUB_STATE_TELEPORT_2) {
		// SUB_STATE_TELEPORT_2 ????????? ???
	}
	else if(substate == SUB_STATE_TELEPORT_3) {
		// SUB_STATE_TELEPORT_3 ????????? ???
	}
...
function onEndCurrentAni_Teleport(obj)
{
	...
	if(substate == SUB_STATE_TELEPORT_0) {
		local horizonDistance = obj.sq_GetIntData(SKILL_TELEPORT, 0); // ???? ???????
		local verticalDistance = obj.sq_GetIntData(SKILL_TELEPORT, 1); // ???? ???????
		...
		obj.sq_IntVectClear();
		obj.sq_IntVectPush(SUB_STATE_TELEPORT_1); // substate????
