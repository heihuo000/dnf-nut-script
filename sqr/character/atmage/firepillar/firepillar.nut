...
function checkCommandEnable_FirePillar(obj)
{
	...
	{
		return obj.sq_IsCommandEnable(SKILL_FIREPILLAR); // ?????忡???? ???????? ????? ????????. ?????:?????? [2012.04.20] obj.sq_IsCommandEnable(SKILL_BROKENARROW);
	}
...
function onSetState_FirePillar(obj, state, datas, isResetTimer)
{
	...
	else if(substate == SUB_STATE_FIREPILLAR_2)
	{
		...
		if(obj.isMyControlObject())
		{
			...
			local arriveTime = obj.sq_GetIntData(SKILL_FIREPILLAR, 0); // 0. ????? ?????? ??? (??????? ??????? ?e?) (1/1000??)
			local attackRate = obj.sq_GetBonusRateWithPassive(SKILL_FIREPILLAR, STATE_FIREPILLAR, 2, 1.0); // 2. ??? ?????(%)
			local distance = obj.sq_GetLevelData(SKILL_FIREPILLAR, 1, skillLevel); // 1.?????? ???
			local term  = obj.sq_GetIntData(SKILL_FIREPILLAR, 2); // 2. ?????? ????

			sq_BinaryStartWrite();
			sq_BinaryWriteDword(attackRate); // ???e????
			sq_BinaryWriteDword(distance); // ?????? ???
			sq_BinaryWriteDword(arriveTime); // ????? ?????? ??? (??????? ??????? ?e?) (1/1000??)
...
function prepareDraw_FirePillar(obj)
{
	...
	if(substate == SUB_STATE_FIREPILLAR_0) {
		// SUB_STATE_FIREPILLAR_0 ????????? ???
	}
	else if(substate == SUB_STATE_FIREPILLAR_1) {
		// SUB_STATE_FIREPILLAR_1 ????????? ???
	}
	else if(substate == SUB_STATE_FIREPILLAR_2) {
		// SUB_STATE_FIREPILLAR_2 ????????? ???
	}
	else if(substate == SUB_STATE_FIREPILLAR_3) {
		// SUB_STATE_FIREPILLAR_3 ????????? ???
	}
	else if(substate == SUB_STATE_FIREPILLAR_4) {
...
function onProcCon_FirePillar(obj)
{
	...
	else if(substate == SUB_STATE_FIREPILLAR_1)
	{
		...
		local grabMaxTime = obj.sq_GetIntData(SKILL_FIREPILLAR, 1); // 1. ????? ??? ??? ???e?
		...
		if((isMeleeAttack == false) || (isMissileObj == true))
		{
			...
			if(direction == sq_GetDirection(attacker))
			{
				...
				if(attackerInfo)
				{
					...
					sq_SetAttackInfoHitDelayRateDamager(attackerInfo, 0.0); // ???? ????????? 0.0???? ??????????..
	...
	if(substate == SUB_STATE_FIREPILLAR_1)
	{
		if(obj.getVar("firepillar").get_vector(VECTOR_FLAG_2))
		{
			...
			if(attackerInfo)
			{
				...
				sq_SetAttackInfoHitDelayRateDamager(attackerInfo, rateDamager); // ???? ????????? 0.0???? ??????????..
