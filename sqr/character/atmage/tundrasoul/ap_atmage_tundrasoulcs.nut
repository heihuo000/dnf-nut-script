...
function proc_appendage_atmage_tundra_cs(appendage)
{
	...
	local t = appendage.sq_var.get_ct_vector(1); // ?????? ????? ?e??? ?????? ????????? ??????.
	local currentT = 0;	
	...
	local frozenWaitTime = appendage.getVar("skl").get_vector(0);	// 0.?????? ????? ?e?
	local frozenLevel = appendage.getVar("skl").get_vector(1);	// 1.??????
	local frozenRate = appendage.getVar("skl").get_vector(2);		// 2.???????
	local frozenTime = appendage.getVar("skl").get_vector(3);		// 3.????e?
		...
		if(!sq_IsValidActiveStatus(obj, ACTIVESTATUS_FREEZE))
		{
			if(appendage.getSource())
			{
				...
				if(attackerObj.isMyControlObject())
				{
					...
					//24259	`Character/Mage/ATTundraSoulFrozen.obj`	// ?????? ??????? ??? ?нú???????
...
function onApplyHpDamage_appendage_atmage_tundra_cs(appendage, newHpDamage, attacker)
{
	...
	if(sq_IsValidActiveStatus(obj, ACTIVESTATUS_FREEZE))
	{
		local frozenAddDamageRate = appendage.getVar("skl").get_vector(4);		// 4.?????? ?? ??? ??????
