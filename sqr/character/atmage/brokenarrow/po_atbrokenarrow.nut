...
BrokenArrow_FIRE_TIME <- 6

function setCustomData_po_ATBrokenArrow(obj, reciveData)
{
	if(!obj) return;
	
	local fireT = reciveData.readDword();
	...
	obj.getVar("dis").push_vector(fireT);
...
//BrokenArrow_state_1 <- 11
	...
	if(passiveState == BrokenArrow_state_0)
	{
		local fireT = obj.getVar("dis").get_vector(BrokenArrow_FIRE_TIME);
		local srcX = obj.getVar("dis").get_vector(BrokenArrow_START_X); // кЫ?x
		local srcY = obj.getVar("dis").get_vector(BrokenArrow_START_Y); // кЫ?y
		local srcZ = obj.getVar("dis").get_vector(BrokenArrow_START_Z); // кЫ?z
		    	
		local dis_x_len = obj.getVar("dis").get_vector(BrokenArrow_MOV_X); // гЫ ьжэиотнч
		local dis_y_len = obj.getVar("dis").get_vector(BrokenArrow_MOV_Y); // yхн ьжэиотнч
		local endZ = obj.getVar("dis").get_vector(BrokenArrow_MOV_Z); // zхн ьжэиотнч
