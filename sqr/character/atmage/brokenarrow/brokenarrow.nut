...
function checkExecutableSkill_BrokenArrow(obj)
{
	...
	local targetObj = sq_GetPassiveObjectByState(obj, 24250, BrokenArrow_state_1); // 24250	`Character/Mage/ATBrokenArrow.obj`			// ?????? - ???Å«??Î¿??(???)
				...
				if(obj.isMyControlObject())
				{
					...
					for(local i = 0;i <= shootNum; i++)
					{
						...
						if (shootNum % 2 == 0)
						{
							...
							{
								obj.sq_SendCreatePassiveObjectPacket(24250, 0, fireOffsetX, fireOffsetY + 1 - (i + 1) / 2 * 10, fireOffsetZ); // È­»ì
							}
							//ù»ÛÉÖª
							else
							{
								obj.sq_SendCreatePassiveObjectPacket(24250, 0, fireOffsetX, fireOffsetY + 1 + i / 2 * 10, fireOffsetZ); // È­»ì
							}
						}
						else
						{
							...
							if (i % 2 == 0)
							{
								obj.sq_SendCreatePassiveObjectPacket(24250, 0, fireOffsetX, fireOffsetY - 4 - i / 2 * 10, fireOffsetZ); // È­»ì
							}
							//ù»ÛÉÖª
							else
							{
								obj.sq_SendCreatePassiveObjectPacket(24250, 0, fireOffsetX, fireOffsetY + 6 + (i - 1) / 2 * 10, fireOffsetZ); // È­»ì
							}
						}
					}
				}
