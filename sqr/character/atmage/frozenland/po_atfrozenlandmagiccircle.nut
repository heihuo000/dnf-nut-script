...
function setState_po_ATFrozenLandMagicCircle(obj, state, datas) {
	...
	} else if (passiveState == S_PO_FROZENLAND_MC_1) {} else if (passiveState == S_PO_FROZENLAND_MC_2) {
		local ani = obj.getCustomAnimation(0);
		obj.setCurrentAnimation(ani);
	} else if (passiveState == S_PO_FROZENLAND_MC_3) {} else if (passiveState == S_PO_FROZENLAND_MC_4) {
...
function onEndCurrentAni_po_ATFrozenLandMagicCircle(obj) {
	...
	local state = obj.getVar("state").get_vector(0);
	local passiveState = state;
	if (passiveState == S_PO_FROZENLAND_MC_4) {
		if (obj.isMyControlObject()) {
			sq_SendDestroyPacketPassiveObject(obj);
		}
	}
