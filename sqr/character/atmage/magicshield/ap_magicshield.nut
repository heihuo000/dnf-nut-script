...
VAR_MAGIC_SHIELD_ICE_HIT_COUNT <- 2; // ????? ??????? ?????? ??? ī??? ???.
...
{		
	...
	if (type == ENUM_ELEMENT_WATER) 
	{
		if (!attacker.isObjectType(0x0821))
		{
			if (!(parentObj.sq_GetTeam() == attacker.getTeam())) 
			{
				local currentIceGuardCount = var.getInt(VAR_MAGIC_SHIELD_ICE_HIT_COUNT); // ????? ??? ???
	...
	else if (type == ENUM_ELEMENT_LIGHT) 
	{
		if (!(parentObj.sq_GetTeam() == attacker.getTeam())) 
		{		
			local stunTime	= sq_GetLevelData(parentObj, SKILL_MAGIC_SHIELD, 4, skill_level); // 4. ?? ???? ?e?
	...
	else if (type == ENUM_ELEMENT_DARK) 
	{
		if (!(parentObj.sq_GetTeam() == attacker.getTeam())) 
		{
			local prob	= sq_GetLevelData(parentObj, SKILL_MAGIC_SHIELD, 5, skill_level)/10.0; // 5. ??? ??? (0.1%)
			local level	= sq_GetLevelData(parentObj, SKILL_MAGIC_SHIELD, 6, skill_level); // 6. ??? ????
			local time	= sq_GetLevelData(parentObj, SKILL_MAGIC_SHIELD, 7, skill_level); // 7. ????e?
				
			sq_sendSetActiveStatusPacket(attacker, parentObj, ACTIVESTATUS_SLOW, prob.tofloat(), level, false, time); // ???, ????, ????, ?e?
		}
	}
}
...
function setMagicShieldType(appendage, obj, type)
{
	...
	var.setAnimation(VAR_MAGIC_SHIELD_BACK_ANI,backAni); // ???? ???? : ??? ???????
...
function onStart_appendage_MagicShield(appendage)
{
	...
	local var = appendage.getVar();	
	local skill_level = sq_GetSkillLevel(parentObj, SKILL_MAGIC_SHIELD);
	local maxIceGuardHit = sq_GetLevelData(parentObj, SKILL_MAGIC_SHIELD, 3, skill_level); // 3. ????? ?????? ??? ???		
	...
	// ?e??? ????
	local skill_level = sq_GetSkillLevel(parentObj,SKILL_MAGIC_SHIELD);
	local time = sq_GetLevelData(parentObj, SKILL_MAGIC_SHIELD, 0, skill_level); // 0. ????e?
