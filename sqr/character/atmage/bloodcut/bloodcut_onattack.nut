...
function onAttackEtc_po_bloodcut(obj,damager, boundingBox, isStuck)
{
	if(!obj)
		return false;
    ...
    if (id == 105)
    {
		local isEventHorizen = obj.getVar("state").get_vector(0);
		local etcID = obj.getVar("state").get_vector(1);
		if(isEventHorizen < 1){
			return;
		}
		if(etcID > 1){
		...
		if (!chr) return;
		if(damager && damager.isObjectType(OBJECTTYPE_ACTIVE) && sq_IsHoldable(chr, damager))
		{
			local masterAppendage = CNSquirrelAppendage.sq_AppendAppendage(damager, chr, -1, false, "character/atmage/moirror/ap_moirror.nut", true);
			if(masterAppendage)
			{
				sq_HoldAndDelayDie(damager, chr, true, false, true, 0, 0, ENUM_DIRECTION_NEUTRAL, masterAppendage);
				masterAppendage.getAppendageInfo().setValidTime(1200);
			}
		}
    }
    else if (id == 224)
	...
	if (id == 65)
	{
		local append = "character/new_mage/cirruscut/ap_cirruscut.nut";
		if(!obj.isMyControlObject()) {
			return;
        	}
        	local mage = obj.getTopCharacter();
		...
		if (obj.getVar("flag").get_vector(0) == 0)
		{
			...
			sq_SendCreatePassiveObjectPacketPos(obj,12257 +  sq_getRandom(0,3).tointeger(), 0, damager.getXPos(), damager.getYPos() + 1,damager.getZPos() + damager.getObjectHeight()/2 );
			if(sq_IsHoldable(mage,damager) && sq_IsGrabable(mage,damager) && !sq_IsFixture(damager))
			{
			
				local appendage = CNSquirrelAppendage.sq_AppendAppendage(damager, mage, SKILL_CIRRUSCUT, false, append, true);
				if(appendage)
				{
					sq_HoldAndDelayDie(damager, mage, true, false, true, 0, 0, ENUM_DIRECTION_NEUTRAL, appendage);
					sq_MoveToAppendageForce(damager, mage, mage, 100, 0, 0, 400, true, appendage,true);
		...
		{
			sq_MoveToAppendageForce(damager, mage, mage, 100, 0, 0, 200, true, appendage,true);
		}
	}
    if (id == 63)
    {
