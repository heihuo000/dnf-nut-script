...
function setCustomDataEtc_po_bloodcut(obj, receiveData)
{
...
//        local ani = obj.getCustomAnimation(163);
    ...
    else if (id == 110)
    {
		...
		if (isBoom > 0)
		{
			obj.setCurrentAnimation(obj.getVar().GetAnimationMap("", "passiveobject/zrr_skill/atmage/animation/ataccident/accident_shock_b03.ani"));
			sq_SetCurrentAttackInfo(obj, sq_GetCustomAttackInfo(obj, 93));
		}
		else
		{
			obj.setCurrentAnimation(obj.getVar().GetAnimationMap("", "passiveobject/zrr_skill/atmage/animation/ataccident/accident_shock_a01.ani"));
    ...
    }else if (id == 109)
    {
		...
		if (isBoom > 0)
		{
			obj.setCurrentAnimation(obj.getVar().GetAnimationMap("", "passiveobject/zrr_skill/atmage/animation/ataccident/accident_box_b01.ani"));
			sq_SetCurrentAttackInfo(obj, sq_GetCustomAttackInfo(obj, 83));
		}
		else
		{
			obj.setCurrentAnimation(obj.getVar().GetAnimationMap("", "passiveobject/zrr_skill/atmage/animation/ataccident/accident_box_a01.ani"));
    ...
    }else if (id == 108)
    {
        ...
        local isBoom = receiveData.readDword();
        local isHorizen = receiveData.readDword(); 
    
		if (isBoom > 0)
		{
			obj.setCurrentAnimation(obj.getVar().GetAnimationMap("", "passiveobject/zrr_skill/atmage/animation/ataccident/accident_magiccircle_b01.ani"));
		}
		else
		{
			obj.setCurrentAnimation(obj.getVar().GetAnimationMap("", "passiveobject/zrr_skill/atmage/animation/ataccident/accident_magiccircle_a01.ani"));
		}
        ...
        obj.getVar("va").push_vector(isHorizen);
    }else if (id == 107)
    {
...
//        local ani = obj.getCustomAnimation(155);
    ...
    }else if (id == 105)//Mirror
    {
        ...
        local isEventHorizen = receiveData.readDword();

        local ani = obj.getVar().GetAnimationMap("", "passiveobject/zrr_skill/atmage/animation/atkaleidoscope/" + etcID + "_attack" + etcID + "_boom.ani"); 
	...
	local attackInfo = sq_GetCustomAttackInfo(obj, 75 + etcID);
        ...
        obj.getVar("state").clear_vector();
        obj.getVar("state").push_vector(isEventHorizen);
        obj.getVar("state").push_vector(etcID);
        obj.getVar("state").push_vector(attackBonusRate);
    }
...
//        local ani = obj.getCustomAnimation(147);
        local ani = obj.getVar().GetAnimationMap("", "passiveobject/zrr_skill/atmage/animation/atdimensionbomb/dimensionbomb_a.ani"); 
    ...
    else if (id == 1003)//particleStrom
    {

        local size = receiveData.readDword();
//        local ani = obj.getCustomAnimation(147);
        //local ani = obj.getVar().GetAnimationMap("", "passiveobject/zrr_skill/atmage/animation/atdimensionbomb/dimensionbomb_a.ani"); 
        local ani = obj.getVar().GetAnimationMap("", "passiveobject/zrr_skill/atmage/animation/atdimensionbomb/dimensionbomb_b_end.ani"); 
    ...
    else if (id == 1004)
    {
		...
		local size = receiveData.readDword();
		local ani = obj.getVar().GetAnimationMap("", "passiveobject/zrr_skill/atmage/animation/atsilhouette/silhouette_eventhorizonboom.ani"); 

		obj.setCurrentAnimation(ani);
		local attackInfo = sq_GetCustomAttackInfo(obj, 113);
		sq_SetCurrentAttackInfo(obj, attackInfo);
		attackInfo = sq_GetCurrentAttackInfo(obj);
		sq_SetCurrentAttackPower(attackInfo, attackBonusRate);
		...
		local currentAni = sq_GetCurrentAnimation(obj);
		local sizeRate = 1.4*size.tofloat()/100.0;
		currentAni.setImageRateFromOriginal(sizeRate, sizeRate);
		currentAni.setAutoLayerWorkAnimationAddSizeRate(sizeRate);
		sq_SetAttackBoundingBoxSizeRate(currentAni, sizeRate, sizeRate, sizeRate);
...
//        local ani = obj.getCustomAnimation(146);
    ...
    }else if (id == 101)//timeTape
    {
        local size = receiveData.readDword();
        local xyProc = receiveData.readDword();
        local xyLevel = receiveData.readDword();
        local xyTime = receiveData.readDword();
        local save = receiveData.readDword();
	local mage = obj.getTopCharacter();
        mage = sq_ObjectToSQRCharacter(mage);	
	local level = sq_GetSkillLevel(mage, SKILL_TIMETAPE);
	size = size + level;
        obj.getVar("sa").clear_vector();
        obj.getVar("sa").push_vector(save);

//        local ani = obj.getCustomAnimation(145);//133 - 144
        local ani = obj.getVar().GetAnimationMap("", 
                    "passiveobject/zrr_skill/atmage/animation/atcorridor/start/corridor_start_up.ani"); 
		obj.setCurrentAnimation(ani);
		local attackInfo = sq_GetCustomAttackInfo(obj, 69);
        sq_SetCurrentAttackInfo(obj, attackInfo);
       // attackInfo = sq_GetCurrentAttackInfo(obj);
       // sq_SetCurrentAttackPower(attackInfo, attackBonusRate);
        ...
        attackInfo = sq_GetCurrentAttackInfo(obj);
        sq_SetCurrentAttackPower(attackInfo, attackBonusRate);
        sq_SetChangeStatusIntoAttackInfo(attackInfo, 0,ACTIVESTATUS_HOLD, xyProc, xyLevel, xyTime);
    ...
    }else if (id == 100)//NewBloodSong
    {
        ...
        obj.getVar("time").push_vector( atkTime * 100 / speed );
        ...
        local id2 = 0;
        if (object.getObjectHeight() > mage.sq_GetIntData(SKILL_BLOODSONG, 27) )
        {
            id2 = 133 + sq_getRandom(0,3);
            obj.getVar("type").push_vector(2);

        }else if (object.getObjectHeight() > mage.sq_GetIntData(SKILL_BLOODSONG, 28) )
        {
            id2 = 137 + sq_getRandom(0,3);
            obj.getVar("type").push_vector(1);
        }else{
            id2 = 141 + sq_getRandom(0,3);
            obj.getVar("type").push_vector(0);

        }
//        local ani = obj.getCustomAnimation(id2);//133 - 144
        local ani = getAtTepesPassiveAnimation(obj,id2);
        ani.setSpeedRate( speed.tofloat() );
        ani.setImageRateFromOriginal(mage.sq_GetIntData(SKILL_BLOODSONG, 29).tofloat() / 100.0
                                    , mage.sq_GetIntData(SKILL_BLOODSONG, 30).tofloat() / 100.0);
        ani.setAutoLayerWorkAnimationAddSizeRate(1.0);

		obj.setCurrentAnimation(ani);

        obj.setCurrentPos(object.getXPos(),object.getYPos(),0);

		local attackInfo = sq_GetCustomAttackInfo(obj, 68);
        sq_SetCurrentAttackInfo(obj, attackInfo);
        attackInfo = sq_GetCurrentAttackInfo(obj);
        sq_SetCurrentAttackBonusRate(attackInfo, attackBonusRate);
        ...
        obj.getVar("obj").clear_obj_vector();
        obj.getVar("obj").push_obj_vector(object);
...
//        local ani = obj.getCustomAnimation(88);
        local ani = obj.getVar().GetAnimationMap("", 
                    "passiveobject/zrr_skill/newmage/animation/blackthorntree/blackthorntree_housestart_00.ani"); 
        obj.setCurrentAnimation(ani);
        local attackInfo = sq_GetCustomAttackInfo(obj, 112);
        sq_SetCurrentAttackInfo(obj, attackInfo);
        attackInfo = sq_GetCurrentAttackInfo(obj);
...
//            ani = obj.getCustomAnimation(87);
	...
	local tChr = obj.getTopCharacter();
	if ( tChr && (sq_GetSkillLevel(tChr, 70) > 0) )
	{
		local attackinfo = sq_GetCurrentAttackInfo(obj);
		if(attackinfo){
    ...
    }else if (id == 65)
    {
       ...
       local ani = obj.getVar().GetAnimationMap("", "passiveobject/zrr_skill/newmage/animation/rosewhip/rosewhipsuccessfront_00.ani"); 
        obj.setCurrentAnimation(ani);
	...
	for (local i = 0; i < objectManager.getCollisionObjectNumber(); i+=1)
	{
		...
		if (object && obj.isEnemy(object) && object.isObjectType(OBJECTTYPE_ACTIVE) )
		{
            ...
            if  (CNSquirrelAppendage.sq_IsAppendAppendage(activeObj, "character/atmage/timetape/ap_timetape.nut"))
            {
                ...
                local XPos = activeObj.sq_findNearLinearMovableXPos(posX, obj.getYPos(),activeObj.getXPos(), activeObj.getYPos(), 10);
