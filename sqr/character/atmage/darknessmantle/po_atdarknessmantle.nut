...
function procCrackDarknessCircle(obj) // ï¿???ï¿?? ï¿??ï¿?? ï¿?? ï¿???ï¿?? ï¿??ï¿??ï¿??ï¿??
{
	if(!obj)
		return;
		
	local circle_obj = obj.getVar().get_obj_vector(0); // ï¿??
...
function setCustomData_po_ATDarknessMantle(obj, reciveData)
{
	...
	local suckVel = reciveData.readDword(); // 0. ï¿??ï¿??ï¿??ï¿?ï¿??
	// 0.ï¿??ï¿??ï¿??ï¿?ï¿??ï¿?? (<-100%->) 1.ï¿???ï¿??ï¿???(%) 2.ï¿???ï¿??ï¿?? 3.ï¿???ï¿??ï¿???ï¿??
	local darkCircleRate = reciveData.readDword(); // ï¿??Ò¼ï¿??ï¿??ï¿?? (100%)
	local expAtkRate = reciveData.readDword(); // 1.ï¿???ï¿??ï¿???(%)
	local slowLevel = reciveData.readDword(); // ï¿??? ï¿??ï¿??
	local slowTime = reciveData.readDword(); // ï¿??? ï¿??ï¿????
	local sizeRate = reciveData.readDword(); // ?ï¿????ï¿??ï¿??ï¿?? ï¿??? ï¿??ï¿?ï¿??Ò¼ï¿???ï¿??ï¿?? (100%)


	// ?ï¿????ï¿??ï¿??ï¿?? ï¿??? ï¿??ï¿?ï¿??Ò¼ï¿???ï¿??ï¿?? (100%)
...
function procAppend_po_ATDarknessMantle(obj)
{
	...
	else if(passiveState == PASSIVEOBJ_SUB_STATE_3)
	{
		...
		{	
			...
			if(objListSize >= 4)
			{		
				local isEndAnimation = true; // ï¿??? ï¿????ï¿???ï¿?? ï¿??ï¿??ï¿??ï¿?? ??
