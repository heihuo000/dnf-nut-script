...
{
	if (!obj) return false;
	local isUse = obj.sq_IsUseSkill(SKILL_ATMAGE_SHARDMAGNUM);

	if (isUse) {
		obj.sq_AddSetStatePacket(STATE_ATMAGE_SHARDMAGNUM , STATE_PRIORITY_USER, false);
		return true;
	}
...
{
    local level = sq_GetSkillLevel(obj, SKILL_ATMAGE_SHARDMAGNUM );		
    if (level > 0)
    {
...
//2
    ...
    if (flagIndex == 10)
    {
        ...
        local atk = obj.sq_GetBonusRateWithPassive(SKILL_ATMAGE_SHARDMAGNUM , STATE_ATMAGE_SHARDMAGNUM , 0, 1.0);
    ...
    if (obj.getVar("flag").get_vector(flagIndex) == 0)
    {
        ...
        {
            local attackBonusRate = obj.sq_GetBonusRateWithPassive(SKILL_ATMAGE_SHARDMAGNUM, STATE_ATMAGE_SHARDMAGNUM, 1, 1.0);
            obj.sq_StartWrite();
...
{	
    ...
    obj.getVar("flag").push_vector(0);
    obj.getVar("flag").push_vector(0);
	local damage = obj.sq_GetBonusRateWithPassive(SKILL_ATMAGE_SHARDMAGNUM , STATE_ATMAGE_SHARDMAGNUM, 0, 1.0);
