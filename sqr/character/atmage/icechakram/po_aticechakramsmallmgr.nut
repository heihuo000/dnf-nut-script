...
function createIceSmallRing(obj, dx, dy, dz)
{
	...
	obj.getVar().push_obj_vector(ring_obj); // ???? 
		...
		if(ringObj)
		{
			if(currentT > maxT)
			{
				if(obj.isMyControlObject())
				{
					...
					//local power = reciveData.readDword(); // 0.???? ??????? ?????? ?????(+)
					
					local multiHitNum = obj.getVar("chakram").get_vector(1); // 1. ???? ???? ??? ?????? ???
...
function setCustomData_po_ATIceChakramSmallMgr(obj, reciveData)
{
	...
	local createSmallRingNum = reciveData.readDword(); // 0. ??????? ??????? ????
	local multiHitNum = reciveData.readDword(); // 1. ???? ???? ??? ?????? ???		
	local power = reciveData.readDword(); // 0.???? ??????? ?????? ?????(+)
