...
function procAppend_po_ATIceChakramSmallIceRing(obj)
{
	...
	{ // ???, ???
		...
		local srcX = obj.getVar("dis").get_vector(SMALLICERING_START_X); // ????x
		local srcY = obj.getVar("dis").get_vector(SMALLICERING_START_Y); // ????y
		local srcZ = obj.getVar("dis").get_vector(SMALLICERING_START_Z); // ????z
		    	
		local dis_x_len = obj.getVar("dis").get_vector(SMALLICERING_MOV_X); // ?? ??????
		local dis_y_len = obj.getVar("dis").get_vector(SMALLICERING_MOV_Y); // y?? ??????
		local endZ = obj.getVar("dis").get_vector(SMALLICERING_MOV_Z); // z?? ??????
	...
	{ // ???
		...
		local attackTime = obj.getVar("attackTime").get_vector(0); // 0.01?????
