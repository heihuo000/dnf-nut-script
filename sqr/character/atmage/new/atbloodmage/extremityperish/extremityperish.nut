...
function checkCommandEnable_ExtremityPerish(obj)
{
    ...
    if (judgeSubBloodValue(obj,obj.sq_GetIntData(SKILL_EXTREMITYPERISH, 6)))
    return false;
	...
	{
		return obj.sq_IsCommandEnable(SKILL_EXTREMITYPERISH);
	}
	return true;
}
...
{	
	...
	if (state == 0)
	{
		setBloodValue(obj,-obj.sq_GetIntData(SKILL_EXTREMITYPERISH, 6));
...
function onKeyFrameFlag_ExtremityPerish(obj, flagIndex)
{
	...
	if (state == 1)
	{
		if (flagIndex == 0)
		{
			//createUIAniWithIDPath(obj,45,"etc/ultimateskillani/atmage_bloodmage_neo.ani");
