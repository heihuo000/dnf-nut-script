...
function setCustomData_po_ATTurnWidmill(obj, reciveData)
{
	...
	obj.getVar("dis").push_vector(disW); // x??????
	obj.getVar("dis").push_vector(disH); // y??????
...
function procAppend_po_ATTurnWidmill(obj)
{
	...
	{
		local fireT = 200;
		local srcX = obj.getVar("dis").get_vector(0); // ????x
		local srcY = obj.getVar("dis").get_vector(1); // ????y
		local srcZ = obj.getVar("dis").get_vector(2); // ????z
		    	
		local distance = obj.getVar("dis").get_vector(3); // ?? ??????
		...
		{
			local disH = obj.getVar("dis").get_vector(4); // ???? ??????
