...
function checkExecutableSkill_fuumashuriken(obj)
{
	...
	if(state == 3||state == 4||state == 5||state == 9||state == 16||state == 40) 
		return false;
	if(obj.sq_IsUseSkill(SKILL_FUUMASHURIKEN))
	{
		...
		if(state == 6 || state == 7 || now_z>40)
		{
			...
			obj.sq_AddSetStatePacket(STATE_FUUMASHURIKEN, STATE_PRIORITY_IGNORE_FORCE, true);
			return true;
		}else{
			obj.sq_IntVectClear();
...
{	
	...
	{
		case 0:
			obj.sq_SetCurrentAnimation(CUSTOM_ANI_FUUMASHURIKEN1);
		break;
		case 1:
			obj.sq_SetCurrentAnimation(CUSTOM_ANI_FUUMASHURIKEN2);
			obj.sq_ZStop();
		break;
	}
}
...
function onAfterSetState_fuumashuriken(obj, state, datas, isResetTimer)
{
	if(!obj) return;
