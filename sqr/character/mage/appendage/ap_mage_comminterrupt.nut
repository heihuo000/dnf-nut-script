...
function mage_comminterrupt(appendage)
{
	...
	local type = sq_getGrowType(obj);
		...
		if(mystate == 0 ||mystate == 3||mystate == 4||mystate == 5||mystate == 9||mystate == 16) return;
	...
	switch (type)
	{
		...
		if(mystate == 6)
		{
			EnableSoften(obj,99,13);
			EnableSoften(obj,101,13);
			EnableSoften(obj,100,13);
			EnableSoften(obj,43,13);
		}
		break;
