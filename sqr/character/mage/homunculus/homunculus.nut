...
function onAfterAttack_TransformFlapper(obj, damager, boundingBox, isStuck) {
	...
	switch (substate) {
	case 2:
		if (sq_GetSkillLevel(obj, 168) < 1) break;
		local isSeal = sq_GetSkill(obj, 168).isSealFunction();
		if (!damager.isObjectType(OBJECTTYPE_ACTIVE) || isSeal) break;
...
function onCreateObject_TransformFlapper(obj, createObject) {
	if (sq_GetSkillLevel(obj, 168) < 1 || sq_GetSkill(obj, 168).isSealFunction()) return;
	local substate = obj.getSkillSubState();
