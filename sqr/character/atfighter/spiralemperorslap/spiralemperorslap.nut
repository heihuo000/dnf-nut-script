...
function onSetState_atfighter_spiralemperorslap(obj, BmCyUtO1E5JVz5jqGb0gPJwa, fPSbwi_m8O, _Jheh8RnJDnYmrkW_Fy6)
{
 if(!obj) return; 
 local TehcNcjGJdafPqB = obj.sq_GetVectorData(fPSbwi_m8O, 0); 
 obj.setSkillSubState(TehcNcjGJdafPqB); 
 switch(TehcNcjGJdafPqB)
 {
 case 0:
 obj.sq_StopMove(); 
 obj.sq_SetCurrentAnimation(145);
 obj.sq_SetCurrentAttackInfo(87);
 ...
 local J_QeRciO3waA = sq_GetSkillLevel(obj, 220); 
 
 local _DZEsjT_IkTeS_hroK2z = obj.sq_GetLevelData(220, 0, J_QeRciO3waA);
 obj.getVar().clear_vector(); 
 obj.getVar().push_vector(_DZEsjT_IkTeS_hroK2z); 
 obj.sq_PlaySound("SPIRAL_EMPEROSLAP_LOOP", 9701); 
 break;
 case 4:
