...
function onSetState_atfighter_chaindrive(obj, laIPNjbuW8UAr0po9Mzf, Adis1AoJPSx1dJ, tfMOmScWjk1KQhveZ)
{
 if(!obj) return; 
 obj.getVar().clear_vector(); 
 local vTNhgVUtkLfw = obj.sq_GetVectorData(Adis1AoJPSx1dJ, 0); 
 obj.setSkillSubState(vTNhgVUtkLfw); 
 switch(vTNhgVUtkLfw)
 {
 case 0:
 ...
 local skill_level = sq_GetSkillLevel(obj, 226); 
 local BevI_21aaRrrOqUDrPe8sZ = sq_GetCurrentAttackInfo(obj); 
 sq_SetChangeStatusIntoAttackInfo(BevI_21aaRrrOqUDrPe8sZ, 0, ACTIVESTATUS_BLEEDING, obj.sq_GetLevelData(226, 3, skill_level),
 obj.sq_GetLevelData(226, 4, skill_level), obj.sq_GetLevelData(226, 5, skill_level),
 obj.sq_GetPowerWithPassive(226, -1, 6, -1, 1.0)); 
 local abxBFb0YHWZVMovF = obj.sq_GetBonusRateWithPassive(226, 226, 0, 1.0); 
 sq_SetCurrentAttackBonusRate(BevI_21aaRrrOqUDrPe8sZ, abxBFb0YHWZVMovF); 
 obj.getVar().push_vector(obj.sq_GetVectorData(Adis1AoJPSx1dJ, 1)); 
 ...
 onSetGrabPos_atfighter_chaindrive(obj, -15, -1, 199, 40); 
 local DcemnYwHtVrxdzc1Aa9c6yFp = obj.sq_GetVectorData(Adis1AoJPSx1dJ, 1); 
 if(DcemnYwHtVrxdzc1Aa9c6yFp == 1)
 obj.getVar().push_vector(215); 
 else if(DcemnYwHtVrxdzc1Aa9c6yFp == 2)
 obj.getVar().push_vector(90); 
 break;
 case 3:
 obj.sq_SetCurrentAnimation(167);
 obj.getVar().push_vector(420); 
 break;
 }
