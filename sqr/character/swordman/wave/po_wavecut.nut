...
function setCustomData_po_wavecut(obj, receiveData) {
	if (!obj) return;
...
/*
function onKeyFrameFlag_po_wavecut(obj, flagIndex)
{

	if (!obj)
		return 0;

    return onKeyFrameFlagEtc_po_wavecut(obj, flagIndex);


}
*/

function onAttack_po_wavecut(obj, damager, boundingBox, isStuck) {
...
/*
function onTimeEvent_po_wavecut(obj, timeEventIndex, timeEventCount)
{
	if (!obj)
		return 0;
    onTimeEventEtc_po_wavecut(obj, timeEventIndex, timeEventCount);
}
*/

function procAppend_po_wavecut(obj) {
	//
	if (!obj) return 0;
	...
	if (id == 125) {
		...
		if (currentT >= 10 && obj.getVar("flag").get_vector(0) == 0) {
			...
			if (maxCount > currCount) {
				...
				sq_SendCreatePassiveObjectPacketPos(obj, 24328, 0, posX, obj.getYPos() + yPos, obj.getZPos());
			}
...
function destroy_po_wavecut(obj) {
...
/*
function onEndCurrentAni_po_wavecut(obj)
{

	if (!obj)
		return 0;
    onEndCurrentAniEtc_po_wavecut(obj);
