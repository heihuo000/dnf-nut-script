...
{
	appendage.sq_AddFunctionName("proc", "proc_appendage_PatrolBomb")
	appendage.sq_AddFunctionName("onStart", "onStart_appendage_PatrolBomb")
	appendage.sq_AddFunctionName("onEnd", "onEnd_appendage_PatrolBomb")
	appendage.sq_AddFunctionName("drawAppend", "drawAppend_appendage_PatrolBomb")
}
...
function drawAppend_appendage_PatrolBomb(appendage, isOver, x, y, isFlip)
{
	if(!appendage) {
		return;		
	}
...
function proc_appendage_PatrolBomb(appendage)
{
	if(!appendage) {
		return;
	}
	...
	if (damager.IsSquirrelAppendage(append))
	{
		...
		local attackBonusRate = obj.sq_GetBonusRateWithPassive(SKILL_PATROLBOMB, -1, 0, holyFlameRate); 
...
function onStart_appendage_PatrolBomb(appendage)
{
	if(!appendage) {
		return;
	}
...
function onEnd_appendage_PatrolBomb(appendage)
{
	if (!appendage) {
		return;
	}	
