...
{
	appendage.sq_AddFunctionName("proc", "proc_appendage_WheelOfPunishment")
	appendage.sq_AddFunctionName("onStart", "onStart_appendage_WheelOfPunishment")
	appendage.sq_AddFunctionName("onEnd", "onEnd_appendage_WheelOfPunishment")
	appendage.sq_AddFunctionName("drawAppend", "drawAppend_appendage_WheelOfPunishment")
}
...
function drawAppend_appendage_WheelOfPunishment(appendage, isOver, x, y, isFlip)
{
	if(!appendage) {
		return;		
	}
...
function proc_appendage_WheelOfPunishment(appendage)
{
	if(!appendage) {
		return;
	}
	...
	if (damager.IsSquirrelAppendage(append))
	{
		...
		local attackBonusRate = obj.sq_GetBonusRateWithPassive(SKILL_WHEELOFPUNISHMENT, -1, 1, holyFlameRate) + obj.sq_GetBonusRateWithPassive(SKILL_WHEELOFPUNISHMENT, -1, 2, holyFlameRate); 
...
function onStart_appendage_WheelOfPunishment(appendage)
{
	if(!appendage) {
		return;
	}
...
function onEnd_appendage_WheelOfPunishment(appendage)
{
	if (!appendage) {
		return;
	}	
