...
{
	appendage.sq_AddFunctionName("proc", "proc_appendage_VirtuousFuryHolyFlame")
	appendage.sq_AddFunctionName("onStart", "onStart_appendage_VirtuousFuryHolyFlame")
	appendage.sq_AddFunctionName("onEnd", "onEnd_appendage_VirtuousFuryHolyFlame")
	appendage.sq_AddFunctionName("drawAppend", "drawAppend_appendage_VirtuousFuryHolyFlame")
}
...
function drawAppend_appendage_VirtuousFuryHolyFlame(appendage, isOver, x, y, isFlip)
{
	if(!appendage) {
		return;		
	}
...
function proc_appendage_VirtuousFuryHolyFlame(appendage)
{
	if(!appendage) {
		return;
	}
	...
	if (damager.IsSquirrelAppendage(append))
	{
		...
		local attackBonusRate = 5 * obj.sq_GetBonusRateWithPassive(SKILL_VIRTUOUSFURY, STATE_VIRTUOUSFURY, 0, holyFlameRate) + obj.sq_GetBonusRateWithPassive(SKILL_VIRTUOUSFURY, STATE_VIRTUOUSFURY, 1, holyFlameRate); 
...
function onStart_appendage_VirtuousFuryHolyFlame(appendage)
{
	if(!appendage) {
		return;
	}
...
function onEnd_appendage_VirtuousFuryHolyFlame(appendage)
{
	if (!appendage) {
		return;
	}	
