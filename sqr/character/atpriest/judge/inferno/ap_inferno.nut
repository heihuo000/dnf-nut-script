...
{
	appendage.sq_AddFunctionName("proc", "proc_appendage_ATInferno")
	appendage.sq_AddFunctionName("onStart", "onStart_appendage_ATInferno")
	appendage.sq_AddFunctionName("onEnd", "onEnd_appendage_ATInferno")
	appendage.sq_AddFunctionName("drawAppend", "drawAppend_appendage_ATInferno")
}
...
function drawAppend_appendage_ATInferno(appendage, isOver, x, y, isFlip)
{
	if(!appendage) {
		return;		
	}
...
function proc_appendage_ATInferno(appendage)
{
	if(!appendage) {
		return;
	}
	...
	if (damager.IsSquirrelAppendage(append))
	{
		...
		local attackBonusRate = 3*obj.sq_GetBonusRateWithPassive(SKILL_INFERNO, -1, 0, holyFlameRate) + obj.sq_GetBonusRateWithPassive(SKILL_INFERNO, -1, 1, holyFlameRate); 
...
function onStart_appendage_ATInferno(appendage)
{
	if(!appendage) {
		return;
	}
...
function onEnd_appendage_ATInferno(appendage)
{
	if (!appendage) {
		return;
	}	
