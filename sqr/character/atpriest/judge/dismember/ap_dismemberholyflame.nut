...
{
	appendage.sq_AddFunctionName("proc", "proc_appendage_DismemberHolyFlame")
	appendage.sq_AddFunctionName("onStart", "onStart_appendage_DismemberHolyFlame")
	appendage.sq_AddFunctionName("onEnd", "onEnd_appendage_DismemberHolyFlame")
	appendage.sq_AddFunctionName("drawAppend", "drawAppend_appendage_DismemberHolyFlame")
}
...
function drawAppend_appendage_DismemberHolyFlame(appendage, isOver, x, y, isFlip)
{
	if(!appendage) {
		return;		
	}
...
function proc_appendage_DismemberHolyFlame(appendage)
{
	if(!appendage) {
		return;
	}
	...
	if (damager.IsSquirrelAppendage(append))
	{
		...
		local attackBonusRate =  3 * obj.sq_GetBonusRateWithPassive(SKILL_DISMEMBER, STATE_DISMEMBER, 0, holyFlameRate) + obj.sq_GetBonusRateWithPassive(SKILL_DISMEMBER, STATE_DISMEMBER, 2, holyFlameRate) + obj.sq_GetBonusRateWithPassive(SKILL_DISMEMBER, STATE_DISMEMBER, 3, holyFlameRate);
...
function onStart_appendage_DismemberHolyFlame(appendage)
{
	if(!appendage) {
		return;
	}
...
function onEnd_appendage_DismemberHolyFlame(appendage)
{
	if (!appendage) {
		return;
	}	
