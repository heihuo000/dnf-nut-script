...
{
	appendage.sq_AddFunctionName("proc", "proc_appendage_FlameOfBaptism")
	appendage.sq_AddFunctionName("onStart", "onStart_appendage_FlameOfBaptism")
	appendage.sq_AddFunctionName("onEnd", "onEnd_appendage_FlameOfBaptism")
	appendage.sq_AddFunctionName("drawAppend", "drawAppend_appendage_FlameOfBaptism")
}
...
function drawAppend_appendage_FlameOfBaptism(appendage, isOver, x, y, isFlip)
{
	if(!appendage) {
		return;		
	}
...
function proc_appendage_FlameOfBaptism(appendage)
{
	if(!appendage) {
		return;
	}
	...
	if (damager.IsSquirrelAppendage(append))
	{
		...
		local attackBonusRate = obj.sq_GetBonusRateWithPassive(SKILL_FLAMEOFBAPTISM, -1, 0, holyFlameRate); 
...
function onStart_appendage_FlameOfBaptism(appendage)
{
	if(!appendage) {
		return;
	}
...
function onEnd_appendage_FlameOfBaptism(appendage)
{
	if (!appendage) {
		return;
	}	
