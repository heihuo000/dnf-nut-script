...
{
	appendage.sq_AddFunctionName("proc", "proc_appendage_CrossOfPunishmentHolyFlame")
	appendage.sq_AddFunctionName("onStart", "onStart_appendage_CrossOfPunishmentHolyFlame")
	appendage.sq_AddFunctionName("onEnd", "onEnd_appendage_CrossOfPunishmentHolyFlame")
	appendage.sq_AddFunctionName("drawAppend", "drawAppend_appendage_CrossOfPunishmentHolyFlame")
}
...
function drawAppend_appendage_CrossOfPunishmentHolyFlame(appendage, isOver, x, y, isFlip)
{
	if(!appendage) {
		return;		
	}
...
function proc_appendage_CrossOfPunishmentHolyFlame(appendage)
{
	if(!appendage) {
		return;
	}
	...
	if (damager.IsSquirrelAppendage(append))
	{
		...
		local attackBonusRate =  obj.sq_GetBonusRateWithPassive(SKILL_CROSS_OF_PUNISHMENT, STATE_CROSS_OF_PUNISHMENT, 0, holyFlameRate) + obj.sq_GetBonusRateWithPassive(SKILL_CROSS_OF_PUNISHMENT, STATE_CROSS_OF_PUNISHMENT, 1, holyFlameRate); 
	...
	if (currTime - appendage.getVar("monsterTime").getInt(0) > time)
	{
		...
		//sq_SendHitObjectPacketByCustomAttackIndex(obj, 10, damager,0,0,sq_GetObjectHeight(damager)/2);
...
function onStart_appendage_CrossOfPunishmentHolyFlame(appendage)
{
	if(!appendage) {
		return;
	}
...
function onEnd_appendage_CrossOfPunishmentHolyFlame(appendage)
{
	if (!appendage) {
		return;
	}	
