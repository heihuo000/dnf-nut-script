...
function checkExecutableSkill_atgunner_dday(obj)
{
	if(!obj) return false; 
	local isUse = obj.sq_IsUseSkill(244); 
	if(isUse) 
	{
		local zPos = obj.getZPos(); 
		local state = obj.getState(); 
		if(zPos > 0 && (state == 6 || state == 7)) 
		{
			obj.sq_IntVectClear();
			obj.sq_IntVectPush(1); 
			obj.sq_AddSetStatePacket(244, STATE_PRIORITY_USER, true); 
		}
		else
		{
			obj.sq_IntVectClear();
