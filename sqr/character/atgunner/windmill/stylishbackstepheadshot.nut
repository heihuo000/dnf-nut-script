...
function onProcCon_atstep_headshot(obj)
{
	if (!obj) return;
...
function onSetState_atstep_headshot(obj, state, datas, isResetTimer)
{	
	if(!obj) return;
...
function onEnterFrame_atstep_headshot(obj, frameIndex)
{
	...
	if(frameIndex == 5)
	{
		...
		obj.sq_SendCreatePassiveObjectPacket(24368, 0, -10, -2, 39); 
		if(WeaponSubType == 0)//ñ§×Ç
		{
			...
			particleCreater.SetPos(sq_GetDistancePos(obj.getXPos(), obj.getDirection(),-10),obj.getYPos()-2, obj.getZPos()+40);
