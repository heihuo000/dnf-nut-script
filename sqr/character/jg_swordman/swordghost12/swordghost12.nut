function checkExecutableSkill_SwordGhost12(obj) {
	if (!obj) return false;
	local isUse = obj.sq_IsUseSkill(SKILL_SWORD_GHOST_12);
	...
	if (state == STATE_SAN_DUAN_ZHAN || state == STATE_SWORD_GHOST_1 || state == STATE_SWORD_GHOST_2 || state == STATE_SWORD_GHOST_7 || state == STATE_SWORD_GHOST_8 || state == STATE_SWORD_GHOST_11 || state == STATE_SWORD_GHOST_15 || state == STATE_SWORD_GHOST_18 || state == STATE_SWORD_GHOST_23) {
		local skill = sq_GetSkill(obj, SKILL_SWORD_GHOST_12);
		//print( "skill:" + skill);
	...
	if (isUse) {
		...
		obj.sq_AddSetStatePacket(STATE_SWORD_GHOST_12, STATE_PRIORITY_IGNORE_FORCE, true);
		return true;
	}
...
function checkCommandEnable_SwordGhost12(obj) {
	if (!obj) return false;
	local state = obj.sq_GetState();
...
function onSetState_SwordGhost12(obj, state, datas, isResetTimer) {
	if (!obj) return;
	obj.sq_StopMove();
...
function onKeyFrameFlag_SwordGhost12(obj, flagIndex) {
	if (!obj) return false;
...
function onEndCurrentAni_SwordGhost12(obj) {
