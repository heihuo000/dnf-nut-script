...
{
	appendage.sq_AddFunctionName("proc", "proc_appendage_monster_358332886_16")
	appendage.sq_AddFunctionName("onStart", "onStart_appendage_monster_358332886_16")
	appendage.sq_AddFunctionName("onEnd", "onEnd_appendage_monster_358332886_16")
	appendage.sq_AddFunctionName("onSourceKeyFrameFlag", "onSourceKeyFrameFlag_appendage_monster_358332886_16");
}

function onSourceKeyFrameFlag_appendage_monster_358332886_16(appendage, flagIndex)
{
	if (!appendage) return;
...
function proc_appendage_monster_358332886_16(appendage)
{
	if (!appendage) return;
...
function onStart_appendage_monster_358332886_16(appendage)
{
	if (!appendage) return;
	local parentObj = appendage.getParent();//ÔğÓğİ«ÓßßÚ
	if (!parentObj)
	...
	for (local i = 0; i < objectNumber; i++)
	{
		...
		local count = object.getMyPassiveObjectCount(109001649);
		if (count > 0)
		{
			local myClone = object.getMyPassiveObject(109001649, 0);
...
function onEnd_appendage_monster_358332886_16(appendage)
{
	if (!appendage) return;
